...
$pageId=3;
//一共有几页
$totalId=selectOne("select count(id) from home_order_shop");
...
$sql="select username,addTime,ordersyn,statusName,orderPrice,o.id,orderStatus,adminopt from home_order_shop o,home_orderstatus_shop as s,
home_user_login as m where o.orderStatus=s.id and o.mid=m.id  {$where} {$searchkeys} order by o.addTime  limit $offset,$pageId";
...
<div class="buyer_day">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="day_list">
        ...
        foreach ($orderList as $k => $v) {
            ...
            <tr>
                ...
                <td class="userpv"><a href="#"><?= $v['username'] ?></a></td>
                <td class="usertime"><?= $v['statusName'] ?></td>
                <td class="userip"><?= $v['orderPrice'] ?></td>
                <td class="websort">
                    <a href="orderAction.php?act=sendGoods&id=<?=$v['id']?>"><?=$v['adminopt']?></a></td>
            </tr>
        <?php
        }
        ?>
    ...
    <?php
        ...
        echo"<h1 style='font-size: 25px;text-align: center;margin-bottom: 40px;'>这一页还没有订单呦</h1>";
    }
    ?>
