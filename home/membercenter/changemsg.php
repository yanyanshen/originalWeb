...
connect();
$userInfo=selectOne("select username,id,qq,moblie,emil,sex from home_user_login where id={$_SESSION['mid']}");
if($userInfo['sex']==1){
    $sex='女';
}else{
    $sex='男';
...
<body>
<div class="main">
    <p class="title"><span>修改个人信息</span></p>
    <form action="changeAction.php?act=changeMsg" method="post" enctype="multipart/form-data">
        <input type="hidden" name="id" value="<?=$userInfo['id']?>"/>
        <table cellpadding="0" cellspacing="0" width="100%">
            <tr><td class="textright">用户名:&nbsp;</td><td><?=$userInfo['username']?></td></tr>
            <tr><td class="textright">用户名:&nbsp;</td><td><?=$sex?></td></tr>
            <tr><td class="textright">手机：</td><td class="pwd"><input name="moblie" value="<?=$userInfo['moblie']?$userInfo['moblie']:''?>" type="text" /></td></tr>
            <tr><td class="textright">QQ：</td><td><input name="qq" value="<?=$userInfo['qq']?$userInfo['qq']:''?>" type="text" /></td></tr>
            <tr><td class="textright">邮箱：</td><td><input name="emil" value="<?=$userInfo['emil']?$userInfo['emil']:''?>" type="text" /></td></tr>
            <tr><td colspan="2"><input type="submit" class="btns" value="保存"/></td></tr>
        </table>
    </form>
</div>
</body>
