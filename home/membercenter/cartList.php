...
<div class="buyer_day box">
    ...
    <form action="../cartAction.php?act=createOrder" method="post" enctype="multipart/form-data" id="form">
        <input type="hidden" name="orderprice" id="orderprice" />
        <table width="80%" border="0" cellpadding="0" cellspacing="0" class="day_list">
            <thead>
            <tr>
                <th class="xuhao"></th>
                <th width="15%">商品图片</th>
            ...
            ?>
            <tr>
                <td class="xuhao" style="color: #ff0000"><input checked="checked" onchange="gettotalprice();" type="checkbox" name="chk[]" value="<?=$v['gid']?>" /></td>
                <td >
                    <a href="../goodsDetail.php?id=<?=$v['gid']?>">
                        <img src="<?=UPLOAD.'/100/thumbImg_100_'.$v['pic']?>" alt="<?=$v['goodsName']?>" style="width:80px;margin:10px;" />
                    </a>
                </td>
                ...
                <td >
                    ...
                    <input type="text" name="buyNum<?=$v['gid']?>" value="<?=$v['buyNum']?>" class="num" id="buynum<?=$k?>" onkeyup="chgnum(this)" />&nbsp;
            ...
            <tr>
                <td colspan="5" id="buy">
                    ...
                    <a href="javascript:document.getElementById('form').submit();" class="tobuy">去结算</a>
                </td>
            </tr>
            </tbody>
        </table>
